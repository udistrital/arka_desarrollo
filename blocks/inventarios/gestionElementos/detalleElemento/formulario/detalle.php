...
class registrarForm {
    ...
    function miForm() {
        ...
        if ($elemento_dep == false) {
            $elemento_dep [0] = array(
                'Información Dependencias' => 'Sin información registrada'
            );
        }
...
//        
        ...
        if ($elemento_traslado == false) {
            $elemento_traslado [0] = array(
                'Traslados' => 'Sin traslados registrados'
            );
        } else {
        ...
        if ($elemento_estado == false) {
            $elemento_estado [0] = array(
                'Estado Elemento' => 'Sin movimientos adicionales registrados'
            );
        }
        ...
        if ($elemento_baja == false) {
            $elemento_baja [0] = array(
                'Bajas Elemento' => 'Sin bajas registradas'
            );
        }
        ...
        $cadenaSql = $this->miSql->getCadenaSql('consultar_clasificacion', $_REQUEST ['elemento']);
        $elemento_clasificacion = $esteRecursoDB->ejecutarAcceso($cadenaSql, "busqueda");

        if ($elemento_clasificacion == false) {
            $elemento_clasificacion [0] = array(
                'Clasificación Elemento' => 'Sin información registrada'
            );
        }
            ...
            unset($atributos);
            {
                ...
                unset($atributos);
                {
                    ...
                    echo $this->miFormulario->agrupacion('inicio', $atributos);
                    {
                        ...
                        if (isset($elemento_traslado)) {
                            // ---------------- CONTROL: Cuadro de Texto --------------------------------------------------------
                            $esteCampo = 'informacion_clasificacion';
                            $atributos ['id'] = $esteCampo;
                            $atributos ['nombre'] = $esteCampo;
                            $atributos ['tipo'] = 'text';
                            $atributos ['estilo'] = 'textoSubtituloCursiva';
                            $atributos ['marco'] = true;
                            $atributos ['estiloMarco'] = '';
                            $atributos ['texto'] = 'Información Clasificación';
                            $atributos ['estilo'] = 'textoNegrita textoAzul';
                            $atributos ["etiquetaObligatorio"] = false;
                            $atributos ['columnas'] = 1;
                            $atributos ['dobleLinea'] = 0;
                            ...
                            foreach ($elemento_clasificacion [0] as $key => $values) {
                                if (!is_numeric($key)) {
                                    $encabezado [$key] = (str_replace("_", " ", $key));
                                }
                            }
                            ...
                            foreach ($elemento_clasificacion [0] as $key => $values) {
                                ...
                                if (!is_numeric($key)) {
                                    ...
                                    $atributos ['estiloMarco'] = '';
                                    $atributos ['etiqueta'] = ucfirst($esteCampo);
                                    $atributos ['texto'] = $elemento_clasificacion [0] [$key];
                                    $atributos ["etiquetaObligatorio"] = false;
                                    $atributos ['columnas'] = 1;
                                    $atributos ['dobleLinea'] = 0;
                                    $atributos ['tabIndex'] = $tab;
