...
//				$cadenaSql .= "WHERE \"FUN_ESTADO\"='A' ";
            ...
            case "consultarElemento" :
                ...
                $cadenaSql = "SELECT ei.*, ad.\"ESF_DEP_ENCARGADA\" dependencia_encargada, espacios.\"ESF_NOMBRE_ESPACIO\" ubicacion_especifica, 
                         tfs.descripcion elemento_estado, fn.\"FUN_NOMBRE\" nombre_funcionario,sa.consecutivo||' - ('||sa.vigencia||')' salidas,
                            tb.descripcion bien_tipo,est.tipo_faltsobr id_tipo_estado_elemento, id_baja baja,ei.funcionario funcionario_encargado,
                        el.descripcion descripcion_elemento ";
                ...
                $cadenaSql .= "LEFT JOIN arka_inventarios.elemento el  ON el.id_elemento=ei.id_elemento_gen ";
                $cadenaSql .= "LEFT JOIN arka_inventarios.salida  sa ON sa.id_salida=ei.id_salida ";
                $cadenaSql .= "LEFT JOIN arka_inventarios.tipo_bienes tb ON tb.id_tipo_bienes=el.tipo_bien ";
                ...
                $cadenaSql .= "LEFT JOIN arka_parametros.arka_funcionarios  fn ON fn.\"FUN_IDENTIFICACION\"=ei.funcionario ";
                $cadenaSql .= " WHERE 1=1 AND el.tipo_bien <> 1 ";
                $cadenaSql .= " AND id_baja IS NULL  ";
                $cadenaSql .= " AND est.tipo_faltsobr IS NULL  ";

                if ($variable [0] != '') {
                    $cadenaSql .= " AND ei.funcionario = '" . $variable [0] . "'";
                }
                if ($variable [1] != '') {
                ...
                if ($variable [3] != '') {
                    $cadenaSql .= " AND  ei.dependencia= '" . $variable [3] . "'";
                }
                if ($variable [4] != '') {
                    $cadenaSql .= " AND  ei.ubicacion= '" . $variable [4] . "'";
                }
