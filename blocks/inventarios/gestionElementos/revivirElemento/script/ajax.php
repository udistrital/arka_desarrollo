...
<script type='text/javascript'>
    ...
    function consultarDependenciaFun(elem, request, response) {
        $.ajax({
            url: "<?php echo $urlFinal16 ?>",
            dataType: "json",
            data: {valor: $("#<?php echo $this->campoSeguro('sede_fun') ?>").val()},
            success: function (data) {


                if (data[0] != " ") {

                    $("#<?php echo $this->campoSeguro('dependencia_fun') ?>").html('');
                    $("<option value=''>Seleccione  ....</option>").appendTo("#<?php echo $this->campoSeguro('dependencia_fun') ?>");
                    $.each(data, function (indice, valor) {

                        $("<option value='" + data[ indice ].ESF_CODIGO_DEP + "'>" + data[ indice ].ESF_DEP_ENCARGADA + "</option>").appendTo("#<?php echo $this->campoSeguro('dependencia_fun') ?>");

                    });

                    $("#<?php echo $this->campoSeguro('dependencia_fun') ?>").removeAttr('disabled');

                    $('#<?php echo $this->campoSeguro('dependencia_fun') ?>').width(270);
                    $("#<?php echo $this->campoSeguro('dependencia_fun') ?>").select2();

                    $("#<?php echo $this->campoSeguro('ubicacion_fun') ?>").val(null);
                    $('#<?php echo $this->campoSeguro('ubicacion_fun') ?>').width(270);
                    $("#<?php echo $this->campoSeguro('ubicacion_fun') ?>").select2();
                    $("#<?php echo $this->campoSeguro('ubicacion_fun') ?>").attr('disabled', '');
    ...
    ;
    function consultarEspacioFun(elem, request, response) {
        $.ajax({
            url: "<?php echo $urlFinal4 ?>",
            dataType: "json",
            data: {valorD: $("#<?php echo $this->campoSeguro('dependencia_fun') ?>").val(),
                valorS: $("#<?php echo $this->campoSeguro('sede_fun') ?>").val(), },
            success: function (data) {
                ...
                if (data[0] != " ") {
                    ...
                    $("#<?php echo $this->campoSeguro('ubicacion_fun') ?>").html('');
                    $("<option value=''>Seleccione  ....</option>").appendTo("#<?php echo $this->campoSeguro('ubicacion_fun') ?>");
                    $.each(data, function (indice, valor) {

                        $("<option value='" + data[ indice ].ESF_ID_ESPACIO + "'>" + data[ indice ].ESF_NOMBRE_ESPACIO + "</option>").appendTo("#<?php echo $this->campoSeguro('ubicacion_fun') ?>");

                    });

                    $("#<?php echo $this->campoSeguro('ubicacion_fun') ?>").removeAttr('disabled');
    ...
    $(function () {
        ...
        $("#<?php echo $this->campoSeguro('sede_fun') ?>").change(function () {
            if ($("#<?php echo $this->campoSeguro('sede_fun') ?>").val() != '') {
                consultarDependenciaFun();
            } else {
                $("#<?php echo $this->campoSeguro('dependencia_fun') ?>").select2();
                $("#<?php echo $this->campoSeguro('dependencia_fun') ?>").attr('disabled', '');
                $("#<?php echo $this->campoSeguro('ubicacion_fun') ?>").select2();
                $("#<?php echo $this->campoSeguro('ubicacion_fun') ?>").attr('disabled', '');
            }
        ...
        $("#<?php echo $this->campoSeguro('dependencia_fun') ?>").change(function () {
            if ($("#<?php echo $this->campoSeguro('dependencia_fun') ?>").val() != '') {
                consultarEspacioFun();
            } else {
                $("#<?php echo $this->campoSeguro('ubicacion_fun') ?>").attr('disabled', '');
            }
