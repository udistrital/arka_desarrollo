    ...
    function getCadenaSql($tipo, $variable = "", $aux = "") {
        ...
        switch ($tipo) {
            ...
            case "consultarAprobadas" :
                ...
                $cadenaSql .= ' nivel, tipo_bien, "PRO_RAZON_SOCIAL" as nombre_proveedor, proveedor, fecha_factura, numero_factura, estado_elemento.fecha_registro  ';
                $cadenaSql .= ' FROM arka_inventarios.estado_elemento  ';
                $cadenaSql .= ' LEFT JOIN arka_inventarios.elemento_individual ON elemento_individual.id_elemento_ind=estado_elemento.id_elemento_ind  ';
                $cadenaSql .= ' LEFT JOIN arka_inventarios.salida On elemento_individual.id_salida=salida.id_salida  ';
                $cadenaSql .= ' LEFT JOIN arka_inventarios.entrada On entrada.id_entrada=salida.id_entrada  ';
                $cadenaSql .= ' LEFT JOIN arka_inventarios.elemento On elemento_individual.id_elemento_gen=elemento.id_elemento ';
                $cadenaSql .= ' LEFT JOIN arka_parametros.arka_funcionarios fun ON fun."FUN_IDENTIFICACION"=elemento_individual.funcionario ';
                ...
                $cadenaSql .= 'AND (tipo_faltsobr=3 OR tipo_faltsobr=2)  AND entrada.estado_entrada = 2 AND id_reposicion=0  ';
