...
//          ventana_emergente.style.display = "block";
            ...
            objetoSPAN.innerHTML = "La siguiente entradas será llevada a documento ajuste:<br><br>" + "ENTRADA | NIT | PROVEEDOR | ID. SUPERVISOR | SUPERVISOR <br><br> " + mensaje;
            $("#ventanaEmergente2").dialog("open");
        });


        var table2 = $('#tablaElementos').DataTable();
        $('#tablaElementos tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
                $(this).removeClass('selected');
            } else {
...
//            $("#ventanaEmergente2").dialog("open","aaa");
//            
            var objetoSPAN2 = document.getElementById('intro3');
            var cadena = [];
            var con = 0;
...
//          ventana_emergente.style.display = "block";
            campos = '';
            vigencias = '';
            table2.rows('.selected').every(function (rowIdx, tableLoop, rowLoop) {
                var data = this.data();
                var texto = (data[1].toString()).toLowerCase() + ", " + data[2].toString().toLowerCase() + ", " + data[3].toString().toLowerCase() +"<br>";

                cadena.push(texto);
                campos = campos + data[0];

            });
            $("#<?php echo $this->campoSeguro('variableId') ?>").val(campos);
            var mensaje = cadena.join(" ");
            objetoSPAN2.innerHTML = "El siguiente elemento sera modificado :<br><br>" + "DESCRIPCIÓN | NIVEL | TIPO BIEN <br><br> " + mensaje;
            $("#ventanaEmergente3").dialog("open");
        });
        
        
        var table3 = $('#tablaElementosCantidad').DataTable();
        $('#tablaElementosCantidad tbody').on('click', 'tr', function () {
            if ($(this).hasClass('selected')) {
                $(this).removeClass('selected');
            } else {
        ...
        $('#btConfirmar2').click(function () {
            $("#confirmar2").hide("fast");
//            $("#ventanaEmergente2").dialog("open","aaa");
//            
            var objetoSPAN3 = document.getElementById('intro4');
            var cadena = [];
            var con = 0;
...
//          ventana_emergente.style.display = "block";
            ...
            vigencias = '';
            cantidad = '';
            table3.rows('.selected').every(function (rowIdx, tableLoop, rowLoop) {
                var data = this.data();
                var cantidad_asignada = table3.cell({row: rowIdx, column: 6}).node();
            
                var texto = (data[1].toString()).toLowerCase() + ", " + data[2].toString().toLowerCase() + ", " + data[3].toString().toLowerCase()+ ", " + data[4].toString().toLowerCase()+ ", " + data[5].toString().toLowerCase()+ ", " +  $('input', cantidad_asignada).val() +"<br>";
            ...
            $("#<?php echo $this->campoSeguro('variablesIdElemento') ?>").val(campos);
            $("#<?php echo $this->campoSeguro('variablesCant') ?>").val(cantidad);
            var mensaje = cadena.join(" ");
            objetoSPAN3.innerHTML = "El siguiente elemento sera modificado :<br><br>" + "IDENTIFICACIÓN | FUNCIONARIO | SEDE | DEPENDENCIA | ESPACIO | CANTIDAD  <br><br> " + mensaje;
            $("#ventanaEmergente4").dialog("open");
        });
