...
class Sql extends \Sql {
    ...
    function getCadenaSql($tipo, $variable = "", $auxiliar = "") {
        ...
        switch ($tipo) {
            ...
            case "consultarCantidadElementosxID" :
                $cadenaSql = "SELECT cantidad_por_asignar ";
            ...
            case "consultarElementoDocumento" :

                $cadenaSql = " SELECT DISTINCT entrada.consecutivo as con, entrada.vigencia as vig, entrada.consecutivo||' - ('||entrada.vigencia||')' consecutivo,entrada.proveedor,pro.\"PRO_RAZON_SOCIAL\" nombre_proveedor, entrada.supervisor, ";
                $cadenaSql .= " fun.\"FUN_NOMBRE\" as nombre_supervisor, sedes.\"ESF_SEDE\" sede, dependencias.\"ESF_DEP_ENCARGADA\" dependencia ";
                ...
                $cadenaSql .= " LEFT JOIN arka_parametros.arka_sedes as sedes ON sedes.\"ESF_ID_SEDE\"=entrada.sede WHERE 1=1 ";
                ...
                $cadenaSql .= " AND entrada.vigencia >= CAST((extract(year from now()))-1 as character varying) ORDER BY entrada.consecutivo";
            ...
            case "consultarElementosxDocumento" :
                $cadenaSql = " SELECT id_elemento, nivel,ce.elemento_nombre as nombre_nivel, tipo_bien,tb.descripcion as nombre_tipo, serie, marca, elemento.descripcion, cantidad, unidad, valor, elemento.iva,ai.descripcion as nombre_iva, subtotal_sin_iva, total_iva, total_iva_con  ";
                $cadenaSql .= " FROM arka_inventarios.elemento  ";
                $cadenaSql .= " LEFT JOIN arka_inventarios.entrada as en ON en.id_entrada=elemento.id_entrada ";
                $cadenaSql .= " LEFT JOIN catalogo.catalogo_elemento as ce ON ce.elemento_id=elemento.nivel ";
                $cadenaSql .= " LEFT JOIN arka_inventarios.aplicacion_iva as ai ON ai.id_iva=elemento.iva ";
                $cadenaSql .= " LEFT JOIN arka_inventarios.tipo_bienes as tb ON tb.id_tipo_bienes=elemento.tipo_bien WHERE 1=1 ";
                if (isset($variable ['entrada']) && $variable ['entrada'] != '') {
                    $cadenaSql .= " AND en.consecutivo = " . $variable ['entrada'] . "";
                }
                if (isset($variable ['vigencia']) && $variable ['vigencia'] != '') {
                    $cadenaSql .= ' AND en.vigencia = ';
                    $cadenaSql .= " '" . $variable ['vigencia'] . "' ";
                }
                $cadenaSql .= "  AND id_elemento NOT IN (SELECT id_elemento FROM arka_inventarios.documento_ajuste) ";

                break;

            case "consultarElementosxDocumentoID" :
                $cadenaSql = " SELECT id_elemento, nivel,ce.elemento_nombre as nombre_nivel, tipo_bien,tb.descripcion as nombre_tipo, serie, marca, elemento.descripcion, cantidad, unidad, valor, elemento.iva,ai.descripcion as nombre_iva, subtotal_sin_iva, total_iva, total_iva_con  ";
                $cadenaSql .= " FROM arka_inventarios.elemento  ";
                $cadenaSql .= " LEFT JOIN arka_inventarios.entrada as en ON en.id_entrada=elemento.id_entrada ";
                $cadenaSql .= " LEFT JOIN catalogo.catalogo_elemento as ce ON ce.elemento_id=elemento.nivel ";
                ...
                if (isset($variable) && $variable != '') {
                    $cadenaSql .= " AND elemento.id_elemento = " . $variable . "";
                }
            ...
            case "consultarElementosInd" :
                $cadenaSql = "  SELECT id_elemento_ind, funcionario id_funcionario, af.\"FUN_NOMBRE\" funcionario,sed.\"ESF_SEDE\" sede,dep.\"ESF_DEP_ENCARGADA\" dependencia, esp.\"ESF_NOMBRE_ESPACIO\" espacio,  elemento_individual.cantidad_asignada";
                $cadenaSql .= " FROM arka_inventarios.elemento_individual   ";
                $cadenaSql .= " JOIN arka_parametros.arka_funcionarios as af ON af.\"FUN_IDENTIFICACION\"=elemento_individual.funcionario   ";
                $cadenaSql .= " LEFT JOIN arka_parametros.arka_espaciosfisicos as esp ON esp.\"ESF_ID_ESPACIO\"=elemento_individual.ubicacion_elemento   ";
                $cadenaSql .= " LEFT JOIN arka_parametros.arka_dependencia as dep ON dep.\"ESF_ID_ESPACIO\"= esp.\"ESF_ID_ESPACIO\"   ";
                $cadenaSql .= " LEFT JOIN arka_parametros.arka_sedes as sed ON sed.\"ESF_COD_SEDE\"=esp.\"ESF_COD_SEDE\"  ";
                $cadenaSql .= "WHERE elemento_individual.id_elemento_gen=" . $variable;
            ...
            case "insertarAjusteDocumento" :
                $cadenaSql = " INSERT INTO ";
                $cadenaSql .= " arka_inventarios.documento_ajuste ";
                $cadenaSql .= "( ";
            ...
            case "eliminarElementoInd" :
                $cadenaSql = " DELETE FROM arka_inventarios_elemento_individual ";
                $cadenaSql .= "  WHERE id_elemento_ind=".$variable;
                break;
            
            case "actualizarElementoInd" :
                $cadenaSql = " UPDATE arka_inventarios.elemento_individual  ";
                $cadenaSql .= " SET cantidad_asignada=".$variable['cantidad'];
                $cadenaSql .= " WHERE id_elemento_ind=".$variable['id_elemento'];
                break;
            
            case "actualizarElementoGen" :
