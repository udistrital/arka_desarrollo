...
if ($_REQUEST ['funcion'] == 'CalItem') {
    ...
    if ($resultadoItems) {
        ...
        $Letras = $funcionLetras->ValorEnLetras($valor, 'Pesos');
...
if ($_REQUEST ['funcion'] == 'vigencia_disponibilidad') {
    ...
    $data = array();
    $cadenaSql = $this->sql->getCadenaSql('vigencia_disponibilidad', $data);
    $resultadoItems = $esteRecursoDBO->ejecutarAcceso($cadenaSql, "busqueda");
    $resultado = json_encode($resultadoItems);
...
if ($_REQUEST ['funcion'] == 'rubros') {
   ...
   $conexion = "sicapital";
    ...
    $data = array();
    $cadenaSql = $this->sql->getCadenaSql('rubros', $data);
    $result = $esteRecursoDBO->ejecutarAcceso($cadenaSql, "busqueda");

    if ($result->rowcount() != 0) {
        foreach ($result as $key => $values) {
            $answer[] = array("id" => $result[$key]['RUB_IDENTIFICADOR'], "text" => $result[$key]['RUB_RUBRO'] . " - " . $result[$key]['RUB_NOMBRE_RUBRO']);
// the text I want to show is in the form of option id - option
        }
    } else {
// 0 results send a message back to say so.
        $answer[] = array("id" => "0", "text" => "No Results 87..");
    }
