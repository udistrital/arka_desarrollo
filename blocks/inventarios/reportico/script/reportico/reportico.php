    ...
    function & derive_meta_value($to_parse) {
        ...
        if (preg_match("/{constant,SW_DB_DRIVER}/", $parsed)) {
            if (defined("SW_DB_TYPE") && SW_DB_TYPE == "framework")
                ...
                $parsed = preg_replace('/{constant,([^}]*)}/', '\1', $parsed);
                if (defined($parsed))
...
//Con o sin menú de administrador
    ...
    var $reportico_url_path = 'blocks/inventarios/script/reportico';
    ...
    var $url_path_to_assets = 'blocks/inventarios/reportico/script/reportico';
    ...
    var $url_path_to_calling_script = 'blocks/inventarios/reportico/script/reportico';
    ...
    function add_criteria_link($clause, $link_from, $link_to, &$query, $link_number = -1) {
        if ($link_number != -1)
            $this->criteria_links[$link_number] = array(
                        "clause" => $clause,
                        "link_from" => $link_from,
                        "tag" => $link_to,
                        "query" => &$query
    ...
    function set_criteria_lookup($query_name, &$lookup_query, $in_table, $in_column) {
        if (array_key_exists($query_name, $this->lookup_queries)) {
            ...
            $this->lookup_queries[$query_name] = new reportico_criteria_column(
                    $query_name, $in_table, $in_column, "CHAR", 0, "###.##", 0
            );
    ...
    function store_column_results() {
        ...
        foreach ($this->columns as $k => $col) {
            $this->columns[$k]->old_column_value = $this->columns[$k]->column_value;
    ...
    function get_execute_mode() {
        ...
        if (array_key_exists('execute_mode', $_REQUEST)) {
            if ($_REQUEST["execute_mode"] == "MAINTAIN" && $this->allow_maintain != "SAFE" && $this->allow_maintain != "FULL" && $this->allow_maintain != "DEMO") {
        ...
        if (array_key_exists('execute_mode', $_REQUEST)) {
            if ($_REQUEST["execute_mode"] == "MAINTAIN" && $this->allow_maintain != "SAFE" && $this->allow_maintain != "DEMO" && $this->allow_maintain != "FULL") {
    ...
    function set_request_columns() {
        ...
        foreach ($this->lookup_queries as $col) {
            ...
            if (get_reportico_session_param("firstTimeIn")) {
                $this->lookup_queries[$col->query_name]->column_value = $this->lookup_queries[$col->query_name]->defaults;
                if (is_array($this->lookup_queries[$col->query_name]->column_value))
                    $this->lookup_queries[$col->query_name]->column_value = implode(",", $this->lookup_queries[$col->query_name]->column_value);
                ...
                if (isset($this->defaults)) {
                    if ($this->lookup_queries[$col->query_name]->criteria_type == "DATERANGE") {
                        if (!convert_date_range_defaults_to_dates("DATERANGE", $this->lookup_queries[$col->query_name]->column_value, $this->lookup_queries[$col->query_name]->column_value, $this->lookup_queries[$col->query_name]->column_value2))
                    ...
                    if ($this->lookup_queries[$col->query_name]->criteria_type == "DATE") {
                        ...
                        if (!convert_date_range_defaults_to_dates("DATE", $this->defaults[0], $this->range_start, $dummy))
                            if (!convert_date_range_defaults_to_dates("DATE", $this->lookup_queries[$col->query_name]->column_value, $this->lookup_queries[$col->query_name]->column_value, $this->lookup_queries[$col->query_name]->column_value2))
        ...
        if ($this->initial_output_style)
            set_reportico_session_param("target_style", $this->initial_output_style);

        // If default starting "show" setting provided by calling framework then use them
        if ($this->show_print_button)
            set_reportico_session_param("show_print_button", ( $this->show_print_button == "show"));
        if ($this->show_refresh_button)
            set_reportico_session_param("show_refresh_button", ( $this->show_refresh_button == "show"));
        if ($this->initial_show_detail)
            set_reportico_session_param("target_show_detail", ( $this->initial_show_detail == "show"));
        if ($this->initial_show_graph)
            set_reportico_session_param("target_show_graph", ( $this->initial_show_graph == "show"));
        if ($this->initial_show_group_headers)
            set_reportico_session_param("target_show_group_headers", ( $this->initial_show_group_headers == "show"));
        if ($this->initial_show_group_trailers)
            set_reportico_session_param("target_show_group_trailers", ( $this->initial_show_group_trailers == "show"));
        if ($this->initial_show_column_headers)
            set_reportico_session_param("target_show_column_headers", ( $this->initial_show_column_headers == "show"));
        if ($this->initial_show_criteria)
            set_reportico_session_param("target_show_criteria", ( $this->initial_show_criteria == "show"));
        ...
        if (get_reportico_session_param("firstTimeIn") && !$this->initial_show_detail && !$this->initial_show_graph && !$this->initial_show_group_headers && !$this->initial_show_group_trailers && !$this->initial_show_column_headers && !$this->initial_show_criteria
        ) {
            ...
            if ($this->execute_mode == "EXECUTE") {
                ...
                if (!$this->target_show_detail && !$this->target_show_graph && !$this->target_show_group_headers && !$this->target_show_group_trailers && !$this->target_show_column_headers && !$this->target_show_criteria) {
        ...
        if (isset($_REQUEST["target_show_detail"]))
            set_reportico_session_param("target_show_detail", $_REQUEST["target_show_detail"]);
        if (isset($_REQUEST["target_show_graph"]))
            set_reportico_session_param("target_show_graph", $_REQUEST["target_show_graph"]);
        if (isset($_REQUEST["target_show_group_headers"]))
            set_reportico_session_param("target_show_group_headers", $_REQUEST["target_show_group_headers"]);
        if (isset($_REQUEST["target_show_group_trailers"]))
            set_reportico_session_param("target_show_group_trailers", $_REQUEST["target_show_group_trailers"]);
        if (isset($_REQUEST["target_show_column_headers"]))
            set_reportico_session_param("target_show_column_headers", $_REQUEST["target_show_column_headers"]);
        if (isset($_REQUEST["target_show_criteria"]))
            set_reportico_session_param("target_show_criteria", $_REQUEST["target_show_criteria"]);
        ...
        foreach ($this->lookup_queries as $col) {
            ...
            if ($this->initial_execution_parameters) {
                if (isset($this->initial_execution_parameters[$col->query_name])) {
                    ...
                    if ($col->criteria_type == "DATERANGE") {
                        if (!convert_date_range_defaults_to_dates("DATERANGE", $criteriaval, $val1, $val2))
            ...
            if ($execute_mode && $this->target_show_criteria &&
                    ( array_key_exists($col->query_name, $_REQUEST) || array_key_exists("MANUAL_" . $col->query_name, $_REQUEST) || array_key_exists("HIDDEN_" . $col->query_name, $_REQUEST)
                    )) {
            ...
            if (array_key_exists($col->query_name, $_REQUEST)) {
                $this->lookup_queries[$col->query_name]->column_value = $_REQUEST[$col->query_name];
            }

            if (array_key_exists("MANUAL_" . $col->query_name, $_REQUEST)) {
                $this->lookup_queries[$col->query_name]->column_value = $_REQUEST["MANUAL_" . $col->query_name];
            ...
            if (array_key_exists($col->query_name . "_FROMDATE_DAY", $_REQUEST)) {
                $this->lookup_queries[$col->query_name]->column_value = $this->lookup_queries[$col->query_name]->collate_request_date(
                        $col->query_name, "FROMDATE", $this->lookup_queries[$col->query_name]->column_value, SW_PREP_DATEFORMAT);
            }

            if (array_key_exists($col->query_name . "_TODATE_DAY", $_REQUEST)) {
                $this->lookup_queries[$col->query_name]->column_value2 = $this->lookup_queries[$col->query_name]->collate_request_date(
                        $col->query_name, "TODATE", $this->lookup_queries[$col->query_name]->column_value2, SW_PREP_DATEFORMAT);
            }

            if (array_key_exists("MANUAL_" . $col->query_name . "_FROMDATE", $_REQUEST)) {
                $this->lookup_queries[$col->query_name]->column_value = $_REQUEST["MANUAL_" . $col->query_name . "_FROMDATE"];
            }

            if (array_key_exists("MANUAL_" . $col->query_name . "_TODATE", $_REQUEST)) {
                $this->lookup_queries[$col->query_name]->column_value2 = $_REQUEST["MANUAL_" . $col->query_name . "_TODATE"];
            }

            if (array_key_exists("EXPANDED_" . $col->query_name, $_REQUEST))
                $this->lookup_queries[$col->query_name]->column_value = $_REQUEST["EXPANDED_" . $col->query_name];
        }
    ...
    function build_query($in_is_expanding = false, $criteria_name = "", $in_design_mode = false) {
        ...
        if ($this->sql_raw) {
            ...
            if ($critwhere) {
                ...
                if ($p->parse()) {
                    if ($p->haswhere) {
                        $this->query_statement = substr($this->query_statement, 0, $p->whereoffset) .
                                " 1 = 1" .
                                $critwhere .
                                " AND" .
    ...
    // -----------------------------------------------------------------------------
    function create_page_header(
    $page_header_name = "", $line, $page_header_text
    ) {
    ...
    // -----------------------------------------------------------------------------
    function create_page_footer(
    $page_footer_name = "", $line, $page_footer_text
    ) {
    ...
    // -----------------------------------------------------------------------------
    function create_group_column(
    $query_name = "", $assoc_column = "", $summary_columns = "", $header_columns = ""
    ) {
    ...
    // -----------------------------------------------------------------------------
    function create_criteria_column(
    $query_name = "", $table_name = "table_name", $column_name = "column_name", $column_type = "string", $column_length = 0, $column_mask = "MASK", $in_select = true
    ) {
        ...
        if ($cl = get_query_column($query_name, $this->columns)) {
            ...
            $this->columns[] = new reportico_criteria_column
                    (
                    $query_name, $table_name, $column_name, $column_type, $column_length, $column_mask, $in_select
            );
            end($this->columns);
            $ky = key($this->columns);
    ...
    // -----------------------------------------------------------------------------
    function set_column_order(
    $query_name = "", $order, $insert_before = true
    ) {
        //echo "=========================================<br>";
        ...
        // Now resort the list
        $n = array_multisort(
                $this->display_order_set["itemno"], SORT_ASC, SORT_NUMERIC, $this->display_order_set["column"]
        );
...
//echo "set table ";
    ...
    function execute_query($in_criteria_name) {
        ...
        for ($_counter = 0; $_counter < 1; $_counter++) {
            ...
            foreach ($this->pre_sql as $sql) {
                ...
                //echo "<BR>$nsql<BR>";
                $recordSet = $conn->Execute($nsql) or handle_error("Pre-Query Failed<BR>$nsql<br><br>" .
                                $conn->ErrorMsg());
                $g_code_area = "";
            }
            ...
            if (!$g_error_status && $conn != false)
                $recordSet = $conn->Execute($this->query_statement) or handle_error("Query Failed<BR><BR>" . $this->query_statement . "<br><br>" .
                                "Status " . $conn->ErrorNo() . " - " .
                                $conn->ErrorMsg());
            if ($conn != false)
    ...
    function fetch_column_attributes() {
        ...
        while ($d = key($this->columns)) {
            ...
            if (array_key_exists($value->query_name, $this->clone_columns)) {
                $value->column_type = $this->clone_columns[$value->query_name][0];
                $value->column_length = $this->clone_columns[$value->query_name][1];
                ...
                foreach ($datadict as $k => $v) {
                    ...
                    if (strtoupper(trim($k)) == strtoupper($value->column_name)) {
                        ...
                        $coldets = $datadict[$k];
                        $value->column_type = reportico_datasource::map_column_type(
                                        $this->datasource->driver, $datadict[$k]->type);
    ...
    function add_assignment
    (
    $query_name, $expression, $criteria
    ) {
    ...
    function changed($query_name) {
        ...
        if ($col->column_value != $col->old_column_value)
            $result = true;
    ...
    function embed_image($column_assignee, $image, $width = false, $height = false) {
        get_query_column($column_assignee, $this->columns)->output_images = array("image" => $image, "width" => $width, "height" => $height);
    }
    ...
    function embed_hyperlink($column_assignee, $label, $url, $open_in_new = true, $is_drilldown = false) {
        get_query_column($column_assignee, $this->columns)->output_hyperlinks = array("label" => $label, "url" => $url, "open_in_new" => $open_in_new, "is_drilldown" => $is_drilldown);
    }
    ...
    function lineno($group_name = false) {
        ...
        if ($group_name) {
            if (!array_key_exists($group_name, $this->groupvals))
                $this->groupvals[$group_name] = array("lineno" => 0);
    ...
    function sum($query_name, $group_name = false) {
        ...
        if ($col->old_column_value && !$col->reset_flag) {
            $result = $col->old_column_value +
                    $col->column_value;
        }
        if ($group_name) {
            if (!array_key_exists($group_name, $col->groupvals))
                $col->groupvals[$group_name] = array("average" => 0,
                            "sum" => "0",
                            "avgct" => 0,
                            "avgsum" => 0,
                            "min" => 0,
...
class reportico_assignment extends reportico_object {
    ...
    function reportico_lookup_string_to_php($in_string) {
        $out_string = preg_replace('/{([^}]*)}/', '\"".$this->lookup_queries[\'\1\']->column_value."\"', $in_string);
        ...
        while ($looping) {
            ...
            if (preg_match($regpat, $out_string, $matches)) {
                ...
                if (!$clause) {
                    ...
                    $out_string = preg_replace("/\{=*$critexp\}/", $clause, $out_string);
    ...
    function reportico_string_to_php($in_string) {
        ...
        $out_string = preg_replace('/{TARGET_STYLE}/', '$this->target_style', $out_string);

        $out_string = preg_replace('/{TARGET_FORMAT}/', '$this->target_format', $out_string);

        $out_string = preg_replace('/old\({([^}]*)},{([^}]*)}\)/', '$this->old("\1")', $out_string);

        $out_string = preg_replace('/old\({([^}]*)}\)/', '$this->old("\1")', $out_string);

        $out_string = preg_replace('/max\({([^}]*)},{([^}]*)}\)/', '$this->max("\1","\2")', $out_string);

        $out_string = preg_replace('/max\({([^}]*)}\)/', '$this->max("\1")', $out_string);

        $out_string = preg_replace('/min\({([^}]*)},{([^}]*)}\)/', '$this->min("\1","\2")', $out_string);

        $out_string = preg_replace('/min\({([^}]*)}\)/', '$this->min("\1")', $out_string);

        $out_string = preg_replace('/avg\({([^}]*)},{([^}]*)}\)/', '$this->avg("\1","\2")', $out_string);

        $out_string = preg_replace('/avg\({([^}]*)}\)/', '$this->avg("\1")', $out_string);

        $out_string = preg_replace('/sum\({([^}]*)},{([^}]*)}\)/', '$this->sum("\1","\2")', $out_string);

        $out_string = preg_replace('/sum\({([^}]*)}\)/', '$this->sum("\1")', $out_string);

        $out_string = preg_replace('/imagequery\(/', '$this->imagequery(', $out_string);

        $out_string = preg_replace('/reset\({([^}]*)}\)/', '$this->reset("\1")', $out_string);

        $out_string = preg_replace('/changed\({([^}]*)}\)/', '$this->changed("\1")', $out_string);

        $out_string = preg_replace('/groupsum\({([^}]*)},{([^}]*)},{([^}]*)}\)/', '$this->groupsum("\1","\2", "\3")', $out_string);

        //$out_string = preg_replace('/count\(\)/', 
        //'$this->query_count', 
        //$out_string);
        $out_string = preg_replace('/lineno\({([^}]*)}\)/', '$this->lineno("\1")', $out_string);

        if (preg_match('/skipline\(\)/', $out_string)) {
            $this->non_assignment_operation = true;
            $out_string = preg_replace('/skipline\(\)/', '$this->skipline()', $out_string);
        }

        if (preg_match('/apply_style\(.*\)/', $out_string)) {
            $this->non_assignment_operation = true;
            $out_string = preg_replace('/apply_style\(/', '$this->apply_style("' . $this->query_name . "\",", $out_string);
        }

        if (preg_match('/embed_image\(.*\)/', $out_string)) {
            $this->non_assignment_operation = true;
            $out_string = preg_replace('/embed_image\(/', '$this->embed_image("' . $this->query_name . "\",", $out_string);
        }

        if (preg_match('/embed_hyperlink\(.*\)/', $out_string)) {
            $this->non_assignment_operation = true;
            $out_string = preg_replace('/embed_hyperlink\(/', '$this->embed_hyperlink("' . $this->query_name . "\",", $out_string);
        }

        $out_string = preg_replace('/lineno\(\)/', '$this->lineno()', $out_string);

        $out_string = preg_replace('/count\({([^}]*)}\)/', '$this->lineno("\1")', $out_string);

        $out_string = preg_replace('/count\(\)/', '$this->lineno()', $out_string);

        $out_string = preg_replace('/{([^}]*)}/',
                //'$this->columns[\'\1\']->column_value', 
                'get_query_column_value(\'\1\', $this->columns)', $out_string);
