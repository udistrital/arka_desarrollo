...
class registrarForm {
	...
	function miForm() {
		...
		$cadenaSql = $this->miSql->getCadenaSql ( 'consultarElementosContratista',$_REQUEST['identificacion'] );
// 		echo $cadenaSql;exit;
		$elementos_contratista = $esteRecursoDB->ejecutarAcceso ( $cadenaSql, "busqueda" );
		
// 		var_dump($elementos_contratista);
		$cadenaSql2 = $this->miSql->getCadenaSql ( 'nombreContratista', $_REQUEST['identificacion']);
		...
		if ($elementos_contratista !== false) {
			...
			if ($elementos_contratista !== false) {
				...
				for($i = 0; $i < count ( $elementos_contratista ); $i ++) {
					...
					$atributos ['seleccionado'] = ($elementos_contratista[$i]['verificar_existencia']=='f')?false:true;
					$atributos ['deshabilitado'] = false;
					$tab ++;
			...
			$valorCodificado .= "&contratista=" . $_REQUEST ['identificacion'];
			...
			$mensaje = "No Se Encontraron<br>Elementos Relacionados al Contratista";
