...
class FormProcessor {
    ...
    function procesarFormulario() {
        ...
        //Debe buscar si hay sesiones antiguas activas para el usuario y suprimirlas
        $parametros['usuario'] = $_REQUEST["usuario"];
        $parametros['dato'] = 'idUsuario';

        $numSesiones = 0;
        $sesionAnterior = $this->miSesion->sesionActiva($parametros);
        $numSesiones = count($sesionAnterior);
        ...
        if ($_REQUEST ['tiempo'] <= time() + $this->miConfigurador->getVariableConfiguracion('expiracion')) {
            ...
            if ($registro) {
                ...
                if ($registro [0] ['clave'] == $variable ["clave"]) {
                    ...
                    // var_dump ( $arreglo );
                    //añadir número de sesiones para alerta
                    
                    $registro[0]['numSesiones']=$numSesiones;
